...
function tools(opt) {
    ...
    const commonForDiffAndRefresh = async (req, res, refresh) => {
        ...
        try {
            ...
            cookie = getCookie(req, opt);

            if (cookie) {

                opt.debug && log.dump({
                    [l + '_rawcookiefound']: cookie
                });
...
async function unpackRawToken(cookie, req, res, opt) {
    ...
    const expireAtAsDate = new Date(Date.parse(expireAt));
