...
function tools(opt) {
    ...
    tool.secure = async (req, res, next) => {
        ...
        let protect = await opt.protect(req, res, opt);
        ...
        protect = await opt.protect(req, res, opt);
    ...
    const commonForDiffAndRefresh = async (req, res, refresh) => {
        ...
        try {
            ...
            raw = getCookie(req, opt);


            if ( ! raw ) {

                raw = await opt.extracttokenifnotfoundincookie(req, opt);
            }

            if (raw) {

                opt.debug && log.dump({
                    [l + '_rawcookiefound']: raw
                });
                ...
                [
                    ...
                    threshold,
                    now
                ] = await unpackRawToken(raw, req, res, opt);
