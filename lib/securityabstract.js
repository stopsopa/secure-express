...
let unpackRawToken, refreshToken, setCookie;
...
function tools(opt) {
    ...
    tool.secure = async (req, res, next) => {
        ...
        if (loginrequest) {
            ...
            try {
                ...
                const done    = await setCookie(res, payload, formrememberme, `normal set`, opt);
        ...
        const cookie = getCookie(req, opt);

        if (cookie) {

            opt.debug && log.dump({
                rawcookiefound: cookie
            });
            ...
            try {
                ...
                let [
                    ...
                    threshold,
                    now
                ] = await unpackRawToken(cookie, req, res, opt);
    ...
    tool.refresh = async (req, res) => {

        const token = req[opt.requestauthfield];

        let
            error = false,
            newCookieJWTTwoDotsToken = false
        ;
...
setCookie = async (res, payload, formrememberme, mode, opt) => {
...
unpackRawToken = async (cookie, req, res, opt) => {

    let [__, formrememberme, expireAt, value] = decodeURIComponent(cookie).match(/^([^:]*)(?::([^:]+))?(?::(.+))?$/);

    if ( ! expireAt ) {

        throw th(`expiraAt can't be empty in cookie: ` + cookie);
    }
...
refreshToken = async (res, opt, payload) => {
    ...
    try {
        ...
        const done    = await setCookie(res, payload, true, `attempt to refresh token`, opt);
...
function wrapCookieValue (value, formrememberme, opt) {
    ...
    t = t.toISOString().substring(0, 19).replace(/[:-]/g, '_');
